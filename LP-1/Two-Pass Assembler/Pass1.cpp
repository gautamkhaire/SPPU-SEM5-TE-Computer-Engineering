...
int main()
{
	...
	while(!fin.eof())
	{
		...
		else if(label != "NAN") // Processing the Symbols here partially
		{
			if(presentST(label))
			{
		...
		if(opcode == "LTORG")
		{
			cout << "  " << label << "\t" << opcode << "\t" << op1 << "\t" << op2 << "\t"; // We are directly doing continue in LTORG block thus we are printing the console part here only
